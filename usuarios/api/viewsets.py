...
class UsuariosViewSet(ModelViewSet):
    ...
    def create(self, request, *arg, **kwargs):
        try:
            user = self.usuario_service.from_dto(request.data)
            self.usuario_service.save_usuario(user)
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            user = self.usuario_service.from_dto_update(request.data, user)
            self.usuario_service.save_usuario(user)
            serializer = UsuarioSerializer(user)
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            user = Usuario.objects.filter(id=kwargs['pk'])[0]
            self.usuario_service.delete_user(user)
